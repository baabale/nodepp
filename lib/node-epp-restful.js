...
logger.log('debug',"Application config: ", appConfig);
...
class Dispatcher {
  constructor(registry) {
    ...
    logger.log('info',"Starting dispatcher for registry",{registry});
    logger.log('info', "Using config to contact registrar", {"config": this.registryConfig});
    ...
    var eppCommand = () => {
      setTimeout(() => {
        this.state.login({
          ...
          function(data) {
            logger.log('debug', "Got login data: ", data.toString());
            return;
          },
          function(error) {
...
app.get('/checkDomain/:domain', function(req, res) {
  ...
  logger.log('debug',"Checking domain " + domain);
  var data = {
    "domain": domain
  };
...
app.post('/:command', function(req, res) {
  ...
  dispatcher.command(command, queryData).then((response) => {
    ...
    logger.log('info',"Completed " + command, {"command": command, "elapsed": diff.toString()});
    res.send(response);
  }, (error) => {
    res.send(500, error);
  });
